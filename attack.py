...
def manipulate_one_model(
    args, net, client_id, global_model=None, indivial_init_model=None
):
    print(f"Manipulating Client {client_id}")
    if args.attack_type == "inv_grad":  # inverse the gradient of client
        start_w = (
            indivial_init_model[client_id].state_dict()
            if indivial_init_model is not None
