...
class FedPACClient(FedClientBase):
    def __init__(
        ...
        self.sizes_label = torch.tensor(self.label_distribution()).to(self.device)

        self.probs_label = (self.sizes_label / len(self.train_loader.dataset)).to(
            self.device
        )
        self.datasize = torch.tensor(len(self.train_loader.dataset)).to(self.device)
